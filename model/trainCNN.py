...
def train():
    ...
    model.save(get_absolute_path('model/pretrained/cnn_classifier_model.keras'))
    
    # Save tokenizer
    with open(get_absolute_path('model/pretrained/tokenizer.pkl'), 'wb') as tok_file:
        pickle.dump(tokenizer, tok_file)
    
    # Save label encoder
    with open(get_absolute_path('model/pretrained/label_encoder.pkl'), 'wb') as le_file:
