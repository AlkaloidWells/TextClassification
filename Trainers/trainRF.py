...
def train():
    ...
    data = pd.read_csv(get_absolute_path('DataSte/product_data.csv'))
    ...
    for max_features in param_grid['tfidf__max_features']:
        for n_estimators in param_grid['classifier__n_estimators']:
            for max_depth in param_grid['classifier__max_depth']:
                print(f"Training with max_features={max_features}, n_estimators={n_estimators}, max_depth={max_depth}...")
                pipeline.set_params(tfidf__max_features=max_features, classifier__n_estimators=n_estimators, classifier__max_depth=max_depth)
    ...
    with open(get_absolute_path('models/fileRF/category_classifier_model.pkl'), 'wb') as model_file:
        pickle.dump(best_pipeline, model_file)
    with open(get_absolute_path('models/fileRF/label_encoder.pkl'), 'wb') as mapping_file:
        pickle.dump(label_encoder, mapping_file)
    with open(get_absolute_path('models/fileRF/category_mapping.pkl'), 'wb') as category_mapping_file:
        pickle.dump(category_mapping, category_mapping_file)
    with open(get_absolute_path('models/fileRF/tfidf_vectorizer.pkl'), 'wb') as tfidf_file:
